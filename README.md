...
This README is for **humans maintaining the rules**. The agent itself reads [`CLAUDE.md`](CLAUDE.md) → [`rules/RULES.md`](rules/RULES.md) directly.
...
1. New Chisel project arrives with `CLAUDE.md`, `rules/`, and [`new-session-prompt.md`](new-session-prompt.md) at the theme root.
...
3. The agent loads `CLAUDE.md` → `RULES.md`, then the matching reference + skill for the task, and works in pause-and-review phases.
